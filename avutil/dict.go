...
func AvDictCount(d *Dictionary) int {
...
func AvDictSetInt(d *Dictionary, key string, value int64, flags int) int {
	Ckey := C.CString(key)
	defer C.free(unsafe.Pointer(Ckey))
...
func AvDictParseString(d *Dictionary, str, key_val_sep, pairs_sep string, flags int) int {
	Cstr := C.CString(str)
...
func AvDictCopy(d *Dictionary, src *Dictionary, flags int) int {
...
func AvDictGetString(d *Dictionary, key_val_sep, pairs_sep byte) (int, string) {
	var Cbuf *C.char
...
func Key(d *DictionaryEntry) string {
	return C.GoString(d.key)
}

func Value(d *DictionaryEntry) string {
